...
from django.utils.module_loading import \
    import_string as import_class_from_dotted_path
...
# TODO list:
#  validate all method schemas against JSONAPI spec and "real" serializer output
#   - sort, filters, pagination only valid for GET?
#   - complete query parameters from filters
#  Implement all the weird relationship methods:
#    - Are all of POST/PATCH/DELETE defined for both toMany and toOne? I think not:
#    - https://jsonapi.org/format/#crud-updating-to-one-relationships shows only PATCH yet
#      the endpoints include PATCH/POST/DELETE: Is this a DJA issue or schema issue?
#    - https://jsonapi.org/format/#crud-updating-to-many-relationships has PATCH/POST/DELETE
#      so this is OK.


class JSONAPISchemaGenerator(SchemaGenerator):
    """
    Extend DRF's SchemaGenerator to implement jsonapischema command
    """
    # TODO: merge this back into DRF and/or override DRF's schemagenerator
    def get_paths(self, request=None):
        ...
        **Replacement** for rest_framework.schemas.openapi.SchemaGenerator.get_paths():
        ...
        #: `expanded_endpoints` is like view_endpoints with two extra fields tacked on:
        #: - 'tack_on' to the operationId to make it unique.
        #: - 'action' copy of current view.action (list/fetch) as this gets reset for each request.
        # TODO: define an endpoint_inspector_cls that extends EndpointEnumerator instead of doing it here.
        expanded_endpoints = []
        ...
        for path, method, view, tack_on, action in expanded_endpoints:
            if not self.has_view_permissions(path, method, view):
                continue
            # kludge to preserve view.action as it changes "globally" for the same ViewSet whether it is used
            # for a collection, item or related serializer. _expand_related sets it based on whether the related
            # field is a toMany collection or toOne item.
    ...
    def _expand_relationships(self, path, method, view):
        ...
        :return:list[tuple(path, method, view, tack_on, action)]
        `tack_on` is appended to the operationId resulting in, for example, `createFooRelationshipsTobar`
        wher `Foo` is the parent and `bar` is the relationship.
        """
        if not view.queryset or not view.queryset.model:
            return [(path, method, view, '', view.action), ]
        result = []
        m = view.queryset.model
        for field in [f for f in dir(m) if not f.startswith('_')]:
            attr = getattr(m, field)
            if isinstance(attr, (ReverseManyToOneDescriptor, ForwardOneToOneDescriptor)):
                action = 'rels' if isinstance(attr, ReverseManyToOneDescriptor) else 'rel'
                result.append(
                    (path.replace('{related_field}', field), method, view,
                     'RelationshipTo' + field.capitalize(), action)
                )
    ...
    def _expand_related(self, path, method, view, view_endpoints):
        ...
        :return:list[tuple(path, method, view, 'related', action)]
        """
        result = []
        ...
        for field, related_serializer in related_fields:
            ...
            if related_view:
                ...
                result.append((path.replace('{related_field}', field), method, related_view,
                               'RelatedTo' + view.__class__.__name__.replace('ViewSet', ''), action))
...
class JSONAPIAutoSchema(AutoSchema):
    ...
    """
    # TODO: This depends on currently unmerged DRF PRs for 3.10 release:
    # https://github.com/encode/django-rest-framework/pull/6670 (3.10 Milestone)
    # https://github.com/encode/django-rest-framework/pull/6674 (3.10 Milestone)
    # https://github.com/encode/django-rest-framework/pull/6680 (3.10 Milestone; not strictly required)
    ...
    def __init__(self, openapi_schema={}):
        ...
        jsonapi_ref['components'] = {
            'schemas': {
                            ...
                            'properties': {
                                'pointer': {
                                    ...
                                    'description': "A [JSON Pointer](https://tools.ietf.org/html/rfc6901) "
                                                   ...
                                                   "in the request document [e.g. `/data` for a primary "
                                                   "data object, or `/data/attributes/title` for a "
                                                   "specific attribute."
                                },
                                'parameter': {
                                    'type': 'string',
                ...
                'id': {
                    ...
                    'description': "Each resource object’s type and id pair MUST "
                                   "[identify](https://jsonapi.org/format/#document-resource-object-identification) "
            ...
            'parameters': {
                ...
                # TODO: deepObject not well defined/supported: https://github.com/OAI/OpenAPI-Specification/issues/1706
                'fields': {
                    'name': 'fields',
                    'in': 'query',
    ...
    def get_operation(self, path, method, action=None):
        ...
        # pagination, filters only apply to GET of collections and items (HEAD is GET without a responseBody)
    ...
    def _post_item(self, operation, path, action):
        ...
        operation['responses'] = {'201': self._get_toplevel_200_response(operation, collection=False)}
        ...
        operation['responses']['204'] = {'description': '[Created]'
                                                        ...
                                                        'with the supplied `id`. No other changes from what was '
                                                        'POSTed.'}
    ...
    def _patch_item(self, operation, path, action):
        ...
        operation['responses'] = {'200': self._get_toplevel_200_response(operation, collection=False)}
    ...
    def _get_request_body(self, path, method, action):
        ...
        if action == 'rels':  # relationships special case: plural request body (data is array of items)
            return {
                'content': {
                    ct: {
                        'schema': {
    ...
    def _add_post_4xx_responses(self, operation):
        ...
        for code, reason in [
            ...
            ('404', '[Related resource does not exist](https://jsonapi.org/format/#crud-creating-responses-404)'),
    ...
    def _add_patch_4xx_responses(self, operation):
        ...
        for code, reason in [
            ...
            ('404', '[Related resource does not exist](https://jsonapi.org/format/#crud-updating-responses-404)'),
            ('409', '[Conflict]([Conflict](https://jsonapi.org/format/#crud-updating-responses-409)'),
        ]:
            operation['responses'][code] = self._failure_response(reason)
    ...
    def _add_delete_responses(self, operation):
        ...
        self._generic_failure_responses(operation)
        for code, reason in [
            ('404', '[Resource does not exist](https://jsonapi.org/format/#crud-deleting-responses-404)'),
        ]:
            operation['responses'][code] = self._failure_response(reason)
