...
class HasClaim(BasePermission, OAuthLibMixin):
    ...
    def __init__(self):
        ...
        log.debug('OIDC enabled: {}, userinfo_url: {}'.format(oauth2_settings.OIDC_ENABLED, self.userinfo_url))
    ...
    def _get_claims_from_authentication_server(self, request):
        ...
        """
        try:
            access_token = AccessToken.objects.select_related("application", "user").get(token=request.auth)
        ...
        try:
            response = requests.get(self.userinfo_url, headers={'authorization': 'Bearer {}'.format(request.auth)})
    ...
    def has_permission(self, request, view):
        ...
        if request.method in self.claims_map:
            log.debug('HasClaim: looking for {} claim {} value "{}"'
                      .format(request.method, self.claim, self.claims_map[request.method]))
        ...
        if (request.method in self.claims_map
            ...
            if request.auth.userinfo is None:
                if self.userinfo_url:
                    request.auth.userinfo = self._get_claims_from_authentication_server(request)
                else:
            ...
            try:
                ...
                if claims_map_entry is None:
                    log.debug('Claim denied (None means False)')  # TODO: change up to use True/False?
                    return False
                if claims_map_entry == '':
                ...
                if type(claims_map_entry) == str:
                    ...
                    if claims_map_entry in claims:
                        log.debug('Claim approved ("{}" is in {})'.format(claims_map_entry, claims))
                        return True
                    else:
                        log.debug('Claim denied ("{}" not in {})'.format(claims_map_entry, claims))
                        return False
                elif type(claims_map_entry) == re.Pattern:
                    if claims_map_entry.match(claim_value):
                        log.debug('Claim approved ("{}" matches {})'.format(claim_value, claims_map_entry))
                        return True
                    else:
                        log.debug('Claim denied ("{}" does not match {})'.format(claim_value, claims_map_entry))
                        return False
                else:
