...
We customize the migration by renaming it from the auto name and: 
1. Move adding the new `Instructor.person` OneToOneField up to the top.
2. Add Python code to move the `Instructor.name` field into `Person.name` and vice-versa. 
...
```
It's probably a good idea to update the test cases for the changed models....
